defmodule Main do
  def traverse(curr_index, end_index, cards, possible, cards_left) do
    if curr_index == end_index do
      ...
      cards
        ...
        IO.puts " card left #{cards_left} curr idx #{curr_index}"
  ...
  def add_cards(curr_index, cards_to_add, cards) do
    if cards_to_add == 0 or curr_index == length(cards) do
      ...
      IO.puts "cards #{Enum.join(cards, "  ")} curr_index #{curr_index}"
...
res = Main.traverse(0, length(possible) - 1, cards, possible, Enum.at(possible, 0))
